...
import { MonitorPlay, AlertTriangle, Map as MapIcon, Shield, LogIn, Menu, X, Activity } from 'lucide-react';
...
export const PublicDisplay = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col relative overflow-hidden">
      ...
      {/* Infinite Live Ticker */}
      <div className="bg-red-600 text-white text-sm font-bold py-2 px-4 flex items-center gap-4 z-50 shadow-lg overflow-hidden">
        <div className="flex-1 overflow-hidden whitespace-nowrap">
          <div className="animate-marquee inline-block">
            {allAlerts.length > 0 
              ? allAlerts.map(a => {
                  const details = a.affected_areas ? `[AFFECTING: ${a.affected_areas.join(', ')}]` : '';
                  return `🚨 ${a.title.toUpperCase()} ${details} • ${a.description} • `;
                }).join('   ') 
              : "✅ NO ACTIVE MAJOR ALERTS • MONITORING SYSTEMS ACTIVE • STAY SAFE •"
            }
          </div>
        </div>
      </div>
      ...
      {/* Main Header */}
      <header className="bg-slate-900/80 backdrop-blur border-b border-slate-800 p-4 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl md:text-5xl font-black text-white tracking-tighter leading-none transition-all duration-300">
        ...
        <div className="flex items-center gap-6">
          ...
          <Link 
            to="/admin"
            className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 md:px-4 md:py-2 text-sm md:text-base rounded-lg font-medium transition-colors flex items-center gap-2"
          >
      ...
      {/* Sidebar Navigation */}
      <div className={`fixed inset-y-0 left-0 z-[2000] w-64 bg-slate-900 border-r border-slate-800 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <main className="flex-1 p-4 lg:p-6 overflow-y-auto">
        <div className="grid grid-cols-12 gap-6 h-full">
          ...
          <div className="col-span-12 lg:col-span-9 flex flex-col gap-6">
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-xl min-h-[500px] relative overflow-hidden">
              ...
              <div className="absolute bottom-4 left-4 right-4 grid grid-cols-2 md:grid-cols-4 gap-4 pointer-events-none">
                <div className="bg-slate-900/90 backdrop-blur p-3 rounded-lg border border-slate-700 shadow-lg pointer-events-auto">
                  <div className="text-slate-400 text-xs uppercase font-bold">Total Alerts</div>
                  <div className="text-2xl font-bold text-white">{allAlerts.length}</div>
                </div>
                <div className="bg-slate-900/90 backdrop-blur p-3 rounded-lg border border-slate-700 shadow-lg pointer-events-auto">
                  <div className="text-slate-400 text-xs uppercase font-bold">Safe Zones</div>
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {allAlerts.length > 0 ? (
                  allAlerts.map(alert => (
                    <div key={alert.id} className={`p-4 rounded-lg border ${alert.severity === 'critical' ? 'bg-red-900/20 border-red-500/50' : 'bg-slate-800 border-slate-700'}`}>
                      ...
                      <p className="text-slate-400 text-xs line-clamp-2 mb-2">{alert.description}</p>
          ...
          <div className="col-span-12 lg:col-span-3 flex flex-col gap-6">
            ...
            {criticalAlert && criticalAlert.safety_guide && (
              <div className="bg-slate-900 rounded-2xl p-6 border border-red-500/30 relative overflow-hidden">
                ...
                <div className="space-y-4 relative z-10">
                  <div>
                    <h4 className="text-green-400 text-xs font-bold uppercase mb-2">DO's</h4>
                  ...
                  <div>
                    <h4 className="text-red-400 text-xs font-bold uppercase mb-2">DON'Ts</h4>
                    <ul className="space-y-1">
            ...
            <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800">
              ...
              <div className="space-y-3">
                <Link to="/drills" className="block p-4 bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors group">
                  <div className="flex justify-between items-start mb-2">
                  ...
                  <h4 className="text-white font-bold">Take Mock Drill</h4>
                  <p className="text-slate-400 text-xs mt-1">Test your preparedness</p>
                </Link>
                
                <Link to="/training" className="block p-4 bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors group">
                  <div className="flex justify-between items-start mb-2">
                  ...
                  <h4 className="text-white font-bold">Safety Videos</h4>
                  <p className="text-slate-400 text-xs mt-1">Learn survival skills</p>
                </Link>
              </div>
            </div>
          </div>
